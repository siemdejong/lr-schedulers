...
from typing import Callable as TCallable, _CallableGenericAlias
...
def get_func_as_string(func):
    return inspect.getsourcelines(func)

@st.cache_data
def plot_schedule(scheduler_name, scheduler_cls: torch.optim.lr_scheduler.LRScheduler, parameters: dict):
    ...
    for parameter in defaults:
        if "lambda" in str(defaults[parameter]):
            ...
            defaults[parameter] = eval(defaults[parameter])
...
def show_config(scheduler_name, scheduler_cls: torch.optim.lr_scheduler.LRScheduler):
    ...
    for parameter_name in signature.parameters:
        ...
        if annotation in (int, float):
            selected_input = st.slider(f"{parameter_name}", value=default, key=f"{scheduler_name}-{parameter_name}")
        elif type(annotation) is tuple:
            selected_input = st.radio(f"{parameter_name}", options=annotation, index=annotation.index(default), key=f"{scheduler_name}-{parameter_name}")
        elif annotation is bool:
            selected_input = st.toggle(f"{parameter_name}", value=default, key=f"{scheduler_name}-{parameter_name}")
        elif annotation is Iterable:
            selected_input = st.text_input(f"{parameter_name}", key=f"{scheduler_name}-{parameter_name}", value=default)
            selected_input = eval(selected_input)
        elif annotation is Callable or type(annotation) is _CallableGenericAlias:
            selected_input = st.text_input(f"{parameter_name}", value=default)
        else:
            selected_input = st.text_input(f"{parameter_name} (not recognized)", disabled=True, value=default)
...
def main():
    ...
    torch_optim_lr_scheduler_members = sorted(inspect.getmembers(torch.optim.lr_scheduler, inspect.isclass), key=lambda x: x[0])
    for member_name, member_cls in torch_optim_lr_scheduler_members:
        if issubclass(member_cls, torch.optim.lr_scheduler.LRScheduler) and member_name not in ["_LRScheduler", "LRScheduler", "ChainedScheduler", "SequentialLR", "ReduceLROnPlateau"]:
